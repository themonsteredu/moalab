...
console.log('\n[표를 합친 뒤 — 갈래 칸으로 가른다]');
{
  /* ⚠️ 37단계는 갈래마다 표를 따로 뒀는데 원장이 써보고 "합쳐" 라고 했다(45단계).
     표가 하나가 되면 이 판의 `갈래별` 이 `신규 기관 발굴 22곳` 한 줄로 뭉개진다 —
     그래서 **표가 아니라 갈래 칸의 값으로** 가른다 */
  const C = [
    col('m1', 'c1', '기관 이름', 'text', null, 1),
    col('m1', 'c2', '진행 상태', 'select', ['연락 전', '계약', '보류'], 2),
    col('m1', 'c3', '기관 갈래', 'select', ['청소년문화의집', '청소년수련관·수련원', '초등학교'], 3),
    col('m1', 'c4', '다음 연락일', 'date', null, 4),
  ];
  const R = [
    row('m1', 'r1', { c1: '보성문화의집', c2: '연락 전', c3: '청소년문화의집', c4: '2026-09-01' }, 1),
    row('m1', 'r2', { c1: '구례문화의집', c2: '연락 전', c3: '청소년문화의집', c4: '2026-09-20' }, 2),
    row('m1', 'r3', { c1: '보성수련원', c2: '계약', c3: '청소년수련관·수련원', c4: '2026-09-01' }, 3),
    row('m1', 'r4', { c1: '순천수련관', c2: '보류', c3: '청소년수련관·수련원', c4: '2026-09-01' }, 4),
    row('m1', 'r5', { c1: '어디초등학교', c2: '연락 전', c3: null, c4: null }, 5),
  ];
  const b = S.buildSalesBoard(
    [{ duty: duty('m1', '신규 기관 발굴'), groupName: '학교·기관 영업' }], C, R, '2026-09-10',
  );

  eq('표 하나인데 갈래만큼 줄이 선다', b.tables.length, 3);
  eq('갈래 순서는 보기 순서를 따른다',
    b.tables.map((t) => t.dutyName), ['청소년문화의집', '청소년수련관·수련원', '갈래 없음']);
  eq('갈래별 개수', b.tables.map((t) => t.total), [2, 2, 1]);
  eq('갈래별 합 == 전체 (갈래 안 고른 줄도 세야 한다)', b.tables.reduce((a, t) => a + t.total, 0), b.total);

  // 누르면 그 갈래로 검색해둔 채 연다 — 22줄에 떨어뜨리면 다시 찾아야 한다
  eq('갈래 줄은 검색어를 들고 간다', b.tables[0].q, '청소년문화의집');
  eq("'갈래 없음' 은 검색어가 없다 (표 전체를 연다)", b.tables[2].q, undefined);
  eq('묶음 이름 자리에 원래 표 이름', b.tables[0].groupName, '신규 기관 발굴');

  // 보류는 '오늘 연락할 곳' 에 안 뜬다 (기존 규칙 그대로)
  eq('오늘 연락할 곳', b.due.map((d) => d.title), ['보성문화의집', '보성수련원']);
  eq('갈래별 연락할 곳 수', b.tables.map((t) => t.due), [1, 1, 0]);

  // 상태 칸을 갈래로 착각하면 `연락 전 3곳` 이 갈래 이름 자리에 온다
  eq('상태 칸은 갈래가 아니다', S.kindCol(C, C[1]).name, '기관 갈래');
  eq('갈래 칸이 없으면 null', S.kindCol([C[0], C[1]], C[1]), null);
}

console.log('\n[갈래 칸이 없는 표는 예전 그대로]');
{
  const C = [
    col('m2', 'a1', '학교', 'text', null, 1),
    col('m2', 'a2', '상태', 'select', ['연락 전'], 2),
    col('m2', 'a3', '다음 연락일', 'date', null, 3),
  ];
  const R = [row('m2', 'q1', { a1: 'A중', a2: '연락 전', a3: '2026-09-01' }, 1)];
  const b = S.buildSalesBoard(
    [{ duty: duty('m2', '제안서 작성·발송'), groupName: '학교·기관 영업' }], C, R, '2026-09-10',
  );
  eq('표 한 줄 그대로', b.tables.map((t) => t.dutyName), ['제안서 작성·발송']);
  eq('검색어 없이 표 전체를 연다', b.tables[0].q, undefined);
  eq('묶음 이름은 중분류', b.tables[0].groupName, '학교·기관 영업');
}
