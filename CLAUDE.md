...
- [ ] **2단계 — 체크리스트로 한 번에 뿌리기.** 나눠주는 일은 매번 비슷하다.
      템플릿(항목 + 기본 담당자 + 기준일 ±N일)을 한 번 만들어두고 버튼 한 번으로 통째로 뿌린다.
      · 저장 전 **미리보기 필수** — 8건이 한꺼번에 생기는 동작이라 잘못 뿌리면 알림도 8통 간다
      · 알림은 **사람당 한 통으로 묶는다** (20건 뿌렸는데 20통 오면 그날로 알림을 꺼버린다)
      · 뿌린 뒤 **묶음 통째로 삭제 / 묶음 전체 N일 미루기** 를 같은 화면에
      · 템플릿을 고쳐도 **이미 뿌린 업무는 안 바뀐다** (뿌리는 순간 사진처럼 굳는다)
