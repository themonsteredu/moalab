...
- [ ] **3단계 — 홈을 '오늘 할 일' 한 화면으로.** 지금 폰 홈은 접히는 섹션이 하나도 없어서
      전부 펼친 채 3~4화면이다. `order-*` 는 순서만 바꿀 뿐 **길이를 1px 도 안 줄인다.**
      · `ui.tsx` 에 접이식 공용 부품 (지금은 같은 '접기' 가 **세 군데에서 다른 방식**으로 구현돼 있다)
      · `myTasks` 에 업무 합류 — 지금 카드 링크가 `/apps/${id}` 하드코딩이라 `href` 칸을 하나 만들어야 한다
      · 마감 타임라인만 `order-3` 하드코딩이라 강사 화면에서 원장용 정보가 3번째로 끼어든다
      · 팀 현황은 자르는 코드가 없어서 사람 수만큼 무한정 길어진다
      · **이 단계만 혼자 배포한다** — 다섯 명이 매일 여는 유일한 화면이다
