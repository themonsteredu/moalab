...
- [ ] **4단계 — 기한 알림 자동.** 매일 아침 8시, 오늘·내일 마감을 담당자별로 묶어 그 사람에게만.
      · `vercel.json` + `/api/cron/due` + `CRON_SECRET`. Vercel 무료 플랜은 **하루 1회**라
        "오늘 마감 N건 · 내일 마감 M건" 을 **한 통에** 담는다
      · 반드시 `kstDateStr()` 로 날짜를 만든다 — 그냥 짜면 알림이 하루씩 밀린다
      · 같이 고칠 것: `sw.js` 가 **없는 파일**(`/icon.svg`)을 알림 아이콘으로 가리킨다,
        `notificationclick` 이 실패하면 창이 안 열린다,
        공지 저장 시 **실제 발송과 무관하게** "알림도 보냈어요" 가 뜬다(키가 없어도, 구독자가 0명이어도)
