...
| 📄 수업계획안 | **첨부 전용 + 판(버전)** — 최신만 크게, 지난 판은 접힘 | `plan_files` |
| 🎓 강의계획서 | 학교 제출용 한 장 서식 작성 → `/print/lecture/[id]` | `lesson_plans`, `lesson_plan_items` |
...
- [x] 프로그램 페이지에 **강의계획서 작성 섹션** (`LecturePlanForm`) — 분류(배지)·목표·도입·
      전개 이미지(활동 화면/활동작품, 설명 포함)·마무리·교구·기타사항을 채운다.
      글은 `moalab.lesson_plans` (앱당 1행, app_id 가 PK), 이미지는 `lesson_plan_items`
      (slot: `step`/`work`, 버킷 `moalab-plans` 의 `plan-{appId}/`)
      ※ 이 두 테이블은 화면 코드보다 먼저 운영 DB에 생겼다 — schema.sql 을 나중에 맞췄다
- [x] `/print/lecture/[id]` — 위에서 채운 그대로 한 장 서식으로. 안 채운 칸은 기본 문구,
      활동 화면 없으면 샘플 이미지, 활동작품 없으면 수업 사진으로 대체.
      내부용 종합 인쇄(`/print/[id]`)와 달리 **외부 제출용**이라 검증·원가는 안 싣는다
- [x] **내용이 적어 한 장이 안 차면 사진 칸이 늘어나 A4 아래까지 채운다**
      (본문 flex + 전개 칸만 flex-1 + `auto-rows-fr`, 최소 30mm · **최대 70mm** —
      남는 높이를 다 먹이면 사진이 세로로 길쭉해져 어색하다. 그 이상 남으면 칸 여백으로 둔다)
- [x] **활동작품이 없으면 그 블록 자체를 안 싣는다** — 활동 화면 블록도 마찬가지.
      빈 제목만 남으면 덜 만든 문서처럼 보인다
- [x] 도입·마무리·교구 기본 문구는 `src/lib/lecture.ts` — 새 계획서에 미리 채워져 목표만 쓰면 된다
- [x] 카드의 `강의계획서` 배지는 **목표를 써서 저장했을 때** 켜진다
- [x] 여는 곳: 프로그램 화면 > 강의계획서 섹션 / 인쇄는 인쇄 시트 안 `강의계획서 열기`
